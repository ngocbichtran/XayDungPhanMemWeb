...
const OrderList = () => {
    ...
    const [paymentFilter, setPaymentFilter] = useState(''); // Mới: State lọc thanh toán
    const [searchTerm, setSearchTerm] = useState('');
    // Mới: Thêm unpaid_orders vào state mặc định
    const [stats, setStats] = useState({ total_orders: 0, total_revenue: 0, pending_orders: 0, unpaid_orders: 0 });

    useEffect(() => {
        fetchOrders();
        fetchStats();
    }, [statusFilter, paymentFilter, searchTerm]); // Mới: Load lại khi đổi trạng thái thanh toán
    ...
    const fetchOrders = async () => {
        try {
            const response = await axios.get(`http://127.0.0.1:8000/api/admin/orders`, {
                params: {
                    status: statusFilter,
                    payment_status: paymentFilter, // Mới: Gửi thêm filter thanh toán
                    search: searchTerm
                }
            });
    ...
    const fetchStats = async () => {
        try {
            const response = await axios.get('http://127.0.0.1:8000/api/admin/orders/stats');
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="bg-white p-5 rounded-xl shadow-sm border-l-4 border-green-500">
                    ...
                    <p className="text-2xl font-extrabold text-green-600">{Number(stats.total_revenue).toLocaleString('vi-VN')} đ</p>
                </div>
                ...
                {/* Mới: Thống kê đơn chưa thanh toán */}
                        ...
                        <tbody className="divide-y divide-gray-200 text-sm">
                            {orders.length > 0 ? orders.map(order => (
                                <tr key={order.id} className="hover:bg-blue-50/30 transition-colors">
                                    {/* Mới: Sử dụng order_code thay vì id */}
                                    ...
                                    <td className="py-4 px-6 text-center">
                                        {/* Vẫn giữ order.id ở link để Backend gọi đúng route (thường route API dùng ID) */}
